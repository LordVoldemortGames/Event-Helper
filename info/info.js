...
const createStatusLabel = () => {
  const formSection = document.querySelector('#new-ticket')?.closest('section') || document.querySelector('#new-ticket-admin')?.closest('section');
  if (!formSection) {
    return null;
  }
...
const initInfoPage = () => {
  const ticketButton = document.getElementById('new-ticket');
  if (!ticketButton) {
    return;
  }
